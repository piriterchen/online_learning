...
function display_tree($chapterid, $expanded,$user, $row = 0, $start = 0) {
  ...
  $tree = new treenode($start,$chapterid,'', $user, '', 1, true, -1, $expanded, $sublist);

  // tell tree to display itself
  $tree->display($chapterid,$user,$row, $sublist);
...
function display_new_post_form($chapterid, $poster, $parent = 0, $area = 1, $message='',$title='hi') {
    global $table_width;
    if($message!=''){
  ...
  <tr>
    <td  height="40" align="right">
      <input type="submit" style="height:100%;width:70px;background:#1E90FF;color:#FFF;font-size:18px;font-family:'微软雅黑';" name="post" value="发表"
              alt="Post Message" onclick="return h_warning(form2);" /> 
    </td> 
