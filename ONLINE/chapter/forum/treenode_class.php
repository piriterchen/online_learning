...
class treenode {
  ...
  public function __construct($postid, $chapterid, $title, $poster, $posted, $children,
    ...
    if(($sublist || $expand) && $children) {
      ...
      $query = "select * from t_chforum_header where parent = '".$postid."' and chapterid = '".$chapterid."' order by posted";
      ...
      for ($count=0; $row = @$result->fetch_assoc(); $count++)  {
        ...
        $this->m_childlist[$count]= new treenode($row['postid'],$row['chapterid'], $row['title'],
  ...
  function display($chapterid,$user,$row, $sublist = false)   {
...
<script>
  ...
  <form id="form" action="store_new_post.php"  method="post" name="form_3">
    <div id="response" style="display:none;">
       <input type="hidden" id="parent" name="parent" value="1"/>
       <input type="hidden"  name="area" value="1"/>
       <input type="hidden"  name="chapterid" value="<?php echo $_SESSION['chapterid']; ?>">
       ...
       <input style="height:30px;background:#1E90FF;color:#FFF;font-size:15px;font-family:'微软雅黑';" value="回复" type="submit" onclick="return f_warning(form_3);"/>
    </div>
  </form> 
<?php
?>
