...
<?php
    ...
    $NOTE_COURSE = $rs_array['m_note'];
    $sql_tea="select * from ".tb_table_teacourse." where m_courseid=$course_id";
    $rs_tea=$conn->query($sql_tea);
    if($rs_tea){
        $tea_array=mysqli_fetch_array($rs_tea);
    }
    $teaid=$tea_array['m_teacherid'];
	$teacher_array=array();
    $sql_teacher="select * from ".tb_table_teacher." where m_teacherid=$teaid";
...
<div class="chapter_info">
    ...
    ?></h3>
        ...
        <h2><?php echo "共有课程 ".count($chapterindex)." 节"; ?></h2>
        </div>
        <div class="col-sm-3 col-md-6 col-lg-6">
        <form action="download/browsedir.php?courseid=<?php echo $course_id;?>" method="post" name="form_downlod">
        <input class="btn-primary" type="submit" name="download" value="文件下载" onclick="return f_warning();">
        </form>
        </div>
        </div> 
		<?php 
		...
		<div class="row p_inside">
			<div class="col-sm-3 col-md-6 col-lg-6 text-center">
				<h3><?php echo "第".$chapterindex[$i]."节";?><span width="30px">&emsp;</span>
                <?php echo $chapter[$i];?></h3>
             </div>
            <div class="col-sm-3 col-md-6 col-lg-6">
                <h3><a href="<?php echo "./chapter/chapter.php?chapterid=$id";?>">进入课程</a>
                </h3>
			</div>
		</div>
		<?php
	...
	<div class="col-sm-3 col-md-4 col-lg-4 teacher_info text-center">
		<h2><?php echo "授课教师：".$teacher_array['m_teacher'];?></h2>
...
<div class="discussion">
	...
	<div class="col-sm-12 col-md-10 d_col col-lg-10">
		<iframe src="<?php echo "./forum_course/forum.php?courseid=$course_id";?>" width="100%" height="100%" name="in" id="iframepage"	frameborder="0" scrolling="no" onload="iFrameHeight()"></iframe>
	</div>
	</div>
	</div>
</div>
