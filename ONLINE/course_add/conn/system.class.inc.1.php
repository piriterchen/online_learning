...
class SepPage{
	...
	function ShowPageCourse($contentname,$utits,$anothersearchstr,$class){
		...
		$str.="&nbsp;&nbsp;&nbsp;&nbsp;";
		$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=1".$anothersearchstr." class=".$class.">首页</a>";
		$str.="&nbsp;";
		if(($this->nowpage-1)<=0){ 
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=1".$anothersearchstr." class=".$class.">上一页</a>";
		}else{
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=".($this->nowpage-1).$anothersearchstr." class=".$class.">上一页</a>";
		}
		$str.="&nbsp;";	
		if(($this->nowpage+1)>=$pagecount){
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=".$pagecount.$anothersearchstr." class=".$class.">下一页</a>";
		}else{
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=".($this->nowpage+1).$anothersearchstr." class=".$class.">下一页</a>";
		}
		$str.="&nbsp;";
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagec=".$pagecount.$anothersearchstr." class=".$class.">尾页</a>";
		if($this->rs==false)			
			return "";
		else
		    return $str;	
	...
	function ShowChapter($sqlstr,$conn,$pagesize,$nowpage){	//定义方法
  	...
  	if(!$this->rs){								//判断如果查询结果为0，则输出如下内容
			...
			while($this->array=mysqli_fetch_array($this->rs)){
				...
				array_push($array_chapterindex,$this->array['m_chapterindex']);
			}			
			array_push($arrays,$array_chapter,$array_image,$array_chaining,$array_numconcern,$array_chapterid,$array_chapterindex);
			return $arrays;
		}
	}
	function ShowPageChapter($contentname,$utits,$anothersearchstr,$class,$courseid){
		$allrs=$this->conn->query($this->sqlstr);		//执行查询语句
		$record=mysqli_num_rows($allrs);
		$pagecount=ceil($record/$this->pagesize);		//计算共有几页
		$str="";
		$str.="共有".$contentname."&nbsp;".$record."&nbsp;".$utits."&nbsp;每页显示&nbsp;".$this->pagesize."&nbsp;".$utits."&nbsp;第&nbsp;".$this->nowpage."&nbsp;页/共&nbsp;".$pagecount."&nbsp;页";
		$str.="&nbsp;&nbsp;&nbsp;&nbsp;";
		$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=1".$anothersearchstr."&courseid=".$courseid." class=".$class.">首页</a>";
		$str.="&nbsp;";
		if(($this->nowpage-1)<=0){ 
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=1".$anothersearchstr."&courseid=".$courseid." class=".$class.">上一页</a>";
		}else{
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=".($this->nowpage-1).$anothersearchstr."&courseid=".$courseid." class=".$class.">上一页</a>";
		}
		$str.="&nbsp;";	
		if(($this->nowpage+1)>=$pagecount){
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=".$pagecount.$anothersearchstr."&courseid=".$courseid." class=".$class.">下一页</a>";
		}else{
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=".($this->nowpage+1).$anothersearchstr."&courseid=".$courseid." class=".$class.">下一页</a>";
		}
		$str.="&nbsp;";
			$str.="<a href=".$_SERVER['PHP_SELF']."?pagep=".$pagecount.$anothersearchstr."&courseid=".$courseid." class=".$class.">尾页</a>";
		if($this->rs==false)			
			return "";
		else
		    return $str;	
