...
<link href="./register/register.css" rel="stylesheet" type="text/css" media="all" />
<!-- start plugins -->
...
<!--查询结果-->
		<div class="searchresult">
			...
			?>
					<tr>
						...
						<td id="text" onclick="window.location.href='<?php echo "./course.php?courseid=$course_id";?>';"style="cursor:pointer" 
                            ...
                            <p id="course_name"><?php 
                        for($j=0;$j<count($newstr);$j++){
                            $myrow_class[0][$i]=str_ireplace($newstr[$j],"<font color='#FF0000'>".$newstr[$j]."</font>",$myrow_class[0][$i]);
                            $myrow_class[2][$i]=str_ireplace($newstr[$j],"<font color='#FF0000'>".$newstr[$j]."</font>",$myrow_class[2][$i]);
                             }
                             echo $myrow_class[0][$i];?></p>
			...
			<table style="padding-bottom:50px">
				<h2> 视频查询结果 </h2>
			<?php
				if(count($myrow_chapter[0])==0||$myrow_chapter==false||$newstr==false){
			?>				
				<tr>
					<td id="text">
						<p id="no_result">暂无查询结果</p>
			...
			?>
					...
					<tr>
                        ...
                        <img src="
                            <?php
                        $courseid=$myrow_chapter[5][$i];
                        $sql="select * from t_course where m_courseid=$courseid";
                        $result=$conn->query($sql);
                        $namearray=mysqli_fetch_array($result);
                        $name=$namearray['m_course'];
                        for($j=0;$j<count($newstr);$j++){
                            $name=str_ireplace($newstr[$j],"<font color='#FF0000'>".$newstr[$j]."</font>",$name);
                        } 
                        $image=$namearray['m_image'];
                        echo $image;                       
                            ?>" />
						</td>
                        <td id="text" onclick="window.location='<?php echo "./chapter/chapter.php?chapterid=$chapter_id";?>';"style="cursor:pointer" 
							onmouseover="this.style.backgroundColor='#B0E0E6'" onmouseout="this.style.backgroundColor=''">
                            <p id="course_name">
                            <?php
                            for($j=0;$j<count($newstr);$j++){
                                $myrow_chapter[0][$i]=str_ireplace($newstr[$j],"<font color='#FF0000'>".$newstr[$j]."</font>",$myrow_chapter[0][$i]); 
                            }
                            echo $name."&nbsp;&nbsp;".$myrow_chapter[0][$i];?></p>
			...
			?>
				<tr>
                    ...
                    <td id="text" onclick="window.location='<?php echo "./course.php?courseid=$course_id";?>';"style="cursor:pointer" 
						onmouseover="this.style.backgroundColor='#B0E0E6'" onmouseout="this.style.backgroundColor=''">
						<p id="course_name"><?php echo $myrow_class[0][$i];?></p>
                        <p id="course_concern"><?php 
                        for($j=0;$j<count($newstr);$j++){
                           $myrow_class[5][$i]=str_ireplace($newstr[$j],"<font color='#FF0000'>".$newstr[$j]."</font>",$myrow_class[5][$i]);
                         }
                         echo "关注人数：".$myrow_class[4][$i]."&nbsp;&nbsp;任课教师：".$myrow_class[5][$i];?></p>
                        <p id="course_content"><?php echo "内容简介：".$unhtml->chinesesubstr($myrow_class[2][$i],'0','100')."..."; ?></p>
					</td>
				</tr>
			<?php	
